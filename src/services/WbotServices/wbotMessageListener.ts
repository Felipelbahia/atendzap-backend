...
const verifyQueue = async (
  ...
  console.trace("Verificando QUEUE");
  ...
  if (queues.length === 1) {
    ...
    if (greetingMessage.length > 1 && sendGreetingMessageOneQueues?.value === "enabled") {
      //check if is the first message of the ticket
...
const handleMessage = async (
  ...
  if (importing) {
    ...
    if (exists > 0 ) {
      ...
      await new Promise(a => setTimeout(a, parseInt(process.env.TIMEOUT_TO_IMPORT_MESSAGE) || 330));
    }
  }
  ...
  try {
    ...
    //await outOfHourQueue(wbot, ticket, msg, scheduleType);
...
const greetingMessage = async (wbot: Session, ticket: Ticket, msg: proto.IWebMessageInfo, whatsapp?: Whatsapp, isGroup?: boolean) => {
  ...
  if (!whatsapp?.queues?.length && !ticket.userId && !isGroup && !msg.key.fromMe) {
    ...
    if (firstMessage && firstMessage.body.includes(whatsapp.greetingMessage)) {
      return;
    }
...
const wbotMessageListener = async (wbot: Session, companyId: number): Promise<void> => {
  ...
  wbot.ev.on("groups.update", (groupUpdates: GroupMetadata[]) => {
    ...
    groupUpdates.forEach(async (group: GroupMetadata) => {
      ...
      let groupImage;
      try {
        groupImage = await wbot.profilePictureUrl(group.id, "image");
      } catch (e) {
        console.log(e);
        groupImage = process.env.FRONTEND_URL + "/nopicture.png";
      }
